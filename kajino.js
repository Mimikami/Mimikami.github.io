(function(参数,使用模式){
	...
	,动作 = {
		点击START:function(){更新操作记录();碰触元素('.prt-start')},
		点击OK:function(){更新操作记录();碰触元素('.prt-ok')},
		点击YES:function(){更新操作记录();碰触元素('.prt-yes')},
		点击NO:function(){更新操作记录();碰触元素('.prt-no')},
		点击HIGH:function(){更新操作记录();碰触元素('.prt-double-select[select=high]')},
		点击LOW:function(){更新操作记录();碰触元素('.prt-double-select[select=low]')},
		保持第1张卡:function(){画布事件('set1');画布事件2(1)},
		保持第2张卡:function(){画布事件('set2');画布事件2(2)},
		保持第3张卡:function(){画布事件('set3');画布事件2(3)},
		保持第4张卡:function(){画布事件('set4');画布事件2(4)},
		保持第5张卡:function(){画布事件('set5');画布事件2(5)}
	}
	,人工知能 = {
		保持卡片:function(){
			var 牌组 = 查阅.牌桌卡片();
	...
	,自动值守 = {
		沉睡:function(呼出动作){var 沉睡时间=参数.模式设定[使用模式].点击动作延迟几秒+Math.random()*参数.模式设定[使用模式].随机增加的延迟秒数;计时器=setTimeout(呼出动作,沉睡时间*1000)},
		牌桌:function(){
			switch(状态机.牌桌阶段){
				case 0:
					...
					if(判断.可START()){
						...
						if(情况.累计牌桌游戏次数 % 10 == 0){
							...
							var oriTime = 情况.脚本启动时间1;
							var gap = (oldTime-oriTime)/60/1000;
							gap = gap - gap % 1;
							gap = gap - gap % 1;
							var money = 情况.总共收入筹码/gap*60;
							money = money - money %1;
							//显示信息(gap,3);
							显示信息(newTime.getHours() + ':' + newTime.getMinutes() + ':' + newTime.getSeconds()+ ' 已开始挂机' +gap + '分钟，共赚钱'+情况.总共收入筹码+'，预计每小时收入' +money,5);
						}
						状态机.牌桌阶段++;
					}
					自动值守.沉睡(自动值守.牌桌);
				...
				case 2:
					if(判断.可YESNO()){
						...
						var oldTime = new Date().getTime();
						var newTime = new Date(oldTime);
						显示信息(newTime.getHours() + ':' + newTime.getMinutes() + ':' + newTime.getSeconds()+ ' 进入双倍',3);
						状态机.牌桌阶段=0;
						情况.累计双倍游戏次数++;
						自动值守.沉睡(自动值守.双倍);
					}else if(判断.可START()){
						var oldTime = new Date().getTime();
						var newTime = new Date(oldTime);
						显示信息(newTime.getHours() + ':' + newTime.getMinutes() + ':' + newTime.getSeconds()+ ' 组牌失败',3);
						状态机.牌桌阶段=0;
						自动值守.牌桌();
					}else{
						自动值守.沉睡(自动值守.牌桌);
	...
	模式设定:[
		{
			...
			赌双倍遇到这些点数就不要继续:[0],
			赌双倍连续获胜几回合后进入谨慎状态:8,
			赌双倍胜筹码达到多少后进入谨慎状态:500000,
			赌双倍谨慎状态下遇到这些点数就不要继续:[7,8,9],
			赌双倍连续获胜几回合后停止:20,
			赌双倍筹码达到多少后停止:2500000,
			允许一站到底:false,
